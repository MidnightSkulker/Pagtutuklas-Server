...
-- Local imports
import Disco.Application      ( applicationInitializer )
import Disco.Site             ( site )
import Handler.Disco.Args     ( parseDiscoConfig, discoConfigOptions )
import State.Disco.DiscoState ( DiscoState ( .. ), getHostPort )
...
main = do
  ...
  config <- parseDiscoConfig versionString buildString pname argv discoConfigOptions
  putStrLn ( "Config has been parsed: " ++ show config )
  -- Convert the Janrain configuration information into Janrain
  -- state information
  state :: DiscoState <- fromCfg config
  ...
  putStrLn "Disco server has completed"
